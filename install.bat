@echo off
REM =====================================================
REM Server Status Monitor - Windows Batch Installer
REM 版本: 1.0.0
REM 作者: wututua
REM 最后更新: 2024-01-05
REM =====================================================
...
set LOG_FILE=%TEMP%\ServerStatusMonitor_install_batch.log
...
if /i "%~1"=="/VERYSILENT" set VERY_SILENT=1 && set SILENT=1
...
REM 执行PowerShell安装脚本
call :LOG_INFO "正在调用PowerShell安装脚本..."

set PS_SCRIPT=deploy-windows.ps1
set PS_ARGS=

if %SILENT% equ 1 set PS_ARGS=%PS_ARGS% -Silent
if %VERY_SILENT% equ 1 set PS_ARGS=%PS_ARGS% -VerySilent
if %QUIET% equ 1 set PS_ARGS=%PS_ARGS% -Quiet

powershell -ExecutionPolicy Bypass -File "%PS_SCRIPT%" %PS_ARGS%

if %errorlevel% neq 0 (
    call :LOG_ERROR "PowerShell安装脚本执行失败"
    if %QUIET% equ 0 pause
    exit /b %errorlevel%
)

call :LOG_INFO "安装脚本执行完成"
...
if %QUIET% equ 0 (
    ...
    echo    访问地址: http://localhost:48877
    echo    日志文件: %LOG_FILE%
    echo =====================================================
    echo.
    pause
